...
const DetailCheckbox = React.memo(({ icon, id, title, value, onChange, extra, extraValue, extraPattern, description, onExtra }: DetailCheckboxProps): JSX.Element => (
  <span className={'flex flex-col gap-3 items-center select-none'}>
    <input type={'checkbox'} id={id} checked={value} className={'hidden peer'} onChange={onChange} />
    <label
      htmlFor={id}
      className={`
    ...
    {
      extra && (document.getElementById(id) as HTMLInputElement)?.checked
        ? (
          <>
            <input
              type={'text'}
            ...
            hover:bg-slate-300 focus:border-slate-300 valid:border-sky-500 focus:valid:border-sky-500 invalid:border-red-500 focus:invalid:border-red-500
              transition-all
            ...
            />
            {
              description
                ? (
                  <div className={'text-sm text-slate-300 hidden peer-checked:block'}>
                    {description}
                  </div>
                )
                : null
