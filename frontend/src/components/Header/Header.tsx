        ...
        <header className={'sticky border-b border-inherit'}>
            <div className={'container mx-auto flex flex-wrap p-5 flex-col md:flex-row items-center'}>
                ...
                <nav className={'md:ml-auto flex flex-wrap items-center text-base justify-center text-center'}>
                    <a className={`mr-2.5 ml-2.5 hover:text-sky-500 ${location === '/about' ? 'text-sky-500' : ''}`} href="/about">공지사항</a>
                    <a className={`mr-2.5 ml-2.5 hover:text-sky-500 ${location === '/achievements' ? 'text-sky-500' : ''}`} href="/achievements">후기</a>
                    <a className={`mr-2.5 ml-2.5 hover:text-sky-500 ${location === '/recruit' ? 'text-sky-500' : ''}`} href="/recruit">신청하기</a>
                </nav>
            </div> 
        </header>
    )
